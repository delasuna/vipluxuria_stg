...
<div id="rodape-content">  
    <div id="col-200">
		...
		<h3>Newsletter</h3>
			...
			<p style="padding-bottom:5px;">Cadastre-se no mailing e receba as nossas atualiza��es atrav�s das newsletters.</p>
...
if ($_REQUEST["acao"] == "assinar") {
	...
	if (email != "") {
		...
		if(!$resultado){
			die("Imposs�vel visualizar o newsletter: " . mysql_error() . '<br>');
		}
		$sts = mysql_query($sql);
		$registros = mysql_num_rows($sts);
	
		if ($registros>0) {
				echo "'<script>alert('E-mail j� cadastrado!');</script>";		
		} else {	
			$sql = " INSERT INTO newsletter (email) VALUES ('" . $email . "');"; 
			$resultado = mysql_query($sql, $conexao);
			echo "'<script>alert('Voc� foi adicionado com sucesso!');</script>";
		}
	}
}
...
if ($_REQUEST["acao"] == "remover") {
	...
	if (email != "") {
		...
		$resultado = mysql_query($sql, $conexao);
		echo "'<script>alert('Voc� foi removido com sucesso!');</script>";
	}

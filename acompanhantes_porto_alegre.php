...
<?
	$sql = " SELECT * FROM seo, tipoSeo Where seo.idTipoSeo = tipoSeo.idTipoSeo AND descricao = 'Home'";  
		...
		<div id="principal">
            <div id="principal-content-full">
            	<div id="coluna-full">
					...
					<div id="titulo" style="width: 550px !important;"><h1>Acompanhantes Porto Alegre-RS</h1></div>
                      ...
                      <ul id="thumbs-full">
						...
						if ($registros>0) {
							while($row = mysql_fetch_array($resultado)) {
								...
								if ($contador < 5) { 
							?>
									<li class="zoom_img"><a href="/perfil/<?=$idMulher?>/<?=str_replace($comAcentos, $semAcentos, $nome)?><? if($sobrenome != "") { echo "-".str_replace(" ", "-", str_replace($comAcentos, $semAcentos, $sobrenome));}?>"><img src="<?="/sistema/content/".$imagemCapa?>" width="112" height="149" /><p class="nome"><?=$nome?> <?=$sobrenome?></p></a></li>
							<? 	} else { ?>
									<li class="last zoom_img"><a href="/perfil/<?=$idMulher?>/<?=str_replace($comAcentos, $semAcentos, $nome)?><? if($sobrenome != "") { echo "-".str_replace(" ", "-", str_replace($comAcentos, $semAcentos, $sobrenome));}?>"><img src="<?="/sistema/content/".$imagemCapa?>" width="112" height="149" /><p class="nome"><?=$nome?> <?=$sobrenome?></p></a></li>
						    <?  	$contador = 0;  
								}

...
<div class="voltar-inicio">
	<a href="#inicio">
		<img src="/imagens/base/seta-topo.png" alt="Retornar Topo da Página" width="30" height="30" border="0" />
	</a>
</div>
...
<?php
	// --- INÍCIO DO TRECHO ATUALIZADO ---

	include("../inc/common.php");

	pageTitle("", "Geração de Arquivo");

	// conexão banco (mantendo sua classe existente)
	$conn = new db();
	$conn->open();

	// captura os IDs enviados
	$lista_arquivo = getParam("sel");

	// se vier array (checkbox múltiplo)
	if (is_array($lista_arquivo)) {
		// garante somente números (segurança)
		$lista_arquivo = array_filter($lista_arquivo, fn($v) => ctype_digit((string)$v));
		$lista_arquivo = implode(",", $lista_arquivo);
	}

	// valida se há itens
	if (!strlen($lista_arquivo)) {
	...
	// Cria arquivo
	$arquivo_nome = "lista_news.txt";
	$abre = fopen($arquivo_nome, "w");

	// cabeçalho
	fputs($abre, "email\r\n");

	// SQL protegido contra falhas (mantendo estrutura original)
	$sql = "SELECT email FROM newsletter WHERE id IN ($lista_arquivo)";
	$rs = new query($conn, $sql);

	while ($rs->getrow()) {
		$email = trim($rs->field("email"));
		fputs($abre, "{$email};\r\n");
	}
...
<a href="newsletters_baixar.php?arquivo=lista_news.txt" class="link">
	Clique aqui para baixar sua lista de newsletters.<br>
</a>

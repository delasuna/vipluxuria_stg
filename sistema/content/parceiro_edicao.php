            ...
            <div id="principal-content">
			    <div id="coluna-esquerda-full">
            		<p> 
						...
						<script language="JavaScript"> 
							function salvar() {
								if (document.frm.descricao.value == "") {
									window.alert("A descrição deve ser informada.");
									document.frm.descricao.focus();
								} else if (document.frm.idParceiroTitulo.value == "") {
									window.alert("O título deve ser informado.");
									document.frm.idParceiroTitulo.focus();
								} else {
									document.frm.submit();
								}
							}
							...
							<table width="100%">
								<tr>
									<td>
										...
										<a class='botao' href='parceiro_lista.php?<?=$retorno?>' target=''>&nbsp;Cancelar &nbsp;</a>				
									</td>
								</tr>
							</table>
						</div>
						...
						/* 	Formulário */
						$form = new Form("frm", "parceiro_salvar.php", "POST", "", "100%");
						...
						$form->addHidden("id",$idParceiro); // chave primária
						...
						$sqlTitulo = "SELECT idParceiroTitulo, titulo FROM parceirotitulo ORDER BY titulo";
						
						$form->addField("Título: *", listboxField($sqlTitulo, "idParceiroTitulo",$idParceiroTitulo,"Selecione um Título", "", ""));
						
						$lista_swf = array("S,Sim","N,Não");						
						$form->addField("SWF: ", radioField($lista_swf, "flagSWF",$flagSWF));
						
						
						$form->addField("Descrição: *", textAreaField("descricao",$descricao,7,114,250));
						$form->addField("","Obs.: É possível inserir códigos fonte no campo descrição acima, mas se adicionar uma imagem inserir apenas o link.");
						
						if($imagem != NULL) {  
							$form->addField("Imagem: ", "<input type='file' name='imagem' size='100'><BR>$imagem<BR><img src='$imagem' width='468' height='60' border='1'>");
						} else {
							$form->addField("Imagem: ", "<input type='file' name='imagem' size='100'>");
						}

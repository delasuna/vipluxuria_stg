...
if (isset($_FILES['imagem']) && !empty($_FILES['imagem']['tmp_name'])) {
    ...
    $novoNome = uniqid('bannercentral3_', true) . '.' . $extensao;
...
$sql->setTable("bannercentral3");
$sql->setKey("idBannerCentral3", getParam("id"), "Number");
...
if (strlen(getParam("id")) > 0) {
    ...
    $destino = "bannercentral_lista_2.php?pagina=" . getParam("pagina");
} else {
    // INSERT
    ...
    $conn->execute($sql->getSQL());
    $destino = "bannercentral_lista_2.php";
}

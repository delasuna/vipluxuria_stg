...
/* diretório de upload */
$uploaddir = "upload_bannertransex/";
...
if (isset($_FILES['imagem']) && !empty($_FILES['imagem']['tmp_name'])) {
    ...
    $novoNome = uniqid('bannertransex_', true) . '.' . $extensao;
...
$sql->setTable("bannerTransex");
$sql->setKey("idBannerTransex", getParam("id"), "Number");
...
if (strlen(getParam("id")) > 0) {
    ...
    $destino = "bannertransex_lista.php?pagina=" . getParam("pagina");
} else {
    // INSERT
    ...
    $conn->execute($sql->getSQL());
    $destino = "bannertransex_lista.php";
}

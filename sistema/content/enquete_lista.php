...
<div id="tudo">
    <div id="conteudo">
        ...
        <div id="titulo">
       		<div id="titulo-content"><h1>Enquete</h1></div>
            <div class="traco"></div>
      	</div>
			    ...
			    <div id="coluna-esquerda-full">
					...
					<script>
						function excluir() {
							if (confirm('Excluir registros selecionados?')) {
								document.frm.action = "enquete_excluir.php";
								document.frm.submit();
							}
						}
					</script>	
					...
					<?php
						...
						if ($iSort) {
							...
							if ($iSort == 2) setSession("sOrder"," order by pergunta" . $sDirection); 
						}

						// Query
						$sql = "SELECT * FROM enquete " . getSession("sOrder");
					...
					<form name="form2" method="post" action="enquete_lista.php">
						<div align="right">
						<table>
							<tr>
					...
					<div class='acoes2'>
						<table width="100%">
							<tr>
								<td>
									<a class='botao' href='enquete_edicao.php'>&nbsp;Novo&nbsp;</a>
					...
					<?php
						...
						$table = new Table("","100%",2);
						$table->addColumnHeader("<input type=\"checkbox\" name=\"checkall\" onclick=\"CheckAll()\">");
						$table->addColumnHeader("Pergunta",true,"100%","L");
						$table->addRow();

						while ($rs->getrow()) {
							$id = $rs->field("idEnquete");
							$table->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\">");
							$table->addData(addLink($rs->field("pergunta"),"enquete_edicao.php?id=$id&pagina=$pg","Editar"));
							$table->addRow();	
						}

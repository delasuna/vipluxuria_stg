...
<div id="tudo">
	<div id="conteudo">
		...
		<div id="titulo">
			<div id="titulo-content"><h1>Cidade</h1></div>
			<div class="traco"></div>
		</div>
				...
				<div id="coluna-esquerda-full">
					...
					<script>
						function excluir() {
							if (confirm('Excluir registros selecionados?')) {
								document.frm.action = "cidade_excluir.php";
								document.frm.submit();
							}
						}
					</script>
					...
					<?php
						...
						if ($iSort) {
							...
							if ($iSort == 2) setSession("sOrder"," order by cidade " . $sDirection); 
							if ($iSort == 3) setSession("sOrder"," order by ordem " . $sDirection); 							
						}

						$sql = "SELECT * FROM cidade " . getSession("sOrder");
					...
					<form name="form2" method="post" action="cidade_lista.php">
						<div align="right">
							<table>
								<tr>
					...
					<div class='acoes2'>
						<table width="100%">
							<tr>
								<td>
									<a class='botao' href='cidade_edicao.php'>&nbsp;Novo&nbsp;</a>
					...
					<?php
						...
						$table->addColumnHeader("Nome",true,"70%","L");
						$table->addColumnHeader("Ordem",true,"30%","L");
						$table->addRow();

						while ($rs->getrow()) {
							$id = $rs->field("idCidade");
							$table->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\">");
							$table->addData(addLink($rs->field("cidade"),"cidade_edicao.php?id=$id&pagina=$pg","Editar registro"));
							$table->addData(addLink($rs->field("ordem"),"cidade_edicao.php?id=$id&pagina=$pg","Editar registro"));
							$table->addRow();	
						}

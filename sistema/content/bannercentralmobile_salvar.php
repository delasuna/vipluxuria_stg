...
/* diretório de upload */
$uploaddir = "upload_bannercentralmobile/";
...
if (isset($_FILES['imagem']) && !empty($_FILES['imagem']['tmp_name'])) {
    ...
    $novoNome = uniqid('bannercentralmobile_', true) . '.' . $extensao;
...
$sql->setTable("bannercentralmobile");
$sql->setKey("idBannerCentralMobile", getParam("id"), "Number");
...
if (strlen(getParam("id")) > 0) {
    ...
    $destino = "bannercentralmobile_lista.php?pagina=" . getParam("pagina");
} else {
    // INSERT
    ...
    $conn->execute($sql->getSQL());
    $destino = "bannercentralmobile_lista.php";
}

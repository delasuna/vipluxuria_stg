        ...
        <div id="principal">
            ...
            <div id="principal-content">
			    <div id="coluna-esquerda-full">
					...
					<script language="JavaScript">
						function excluir() {
							if (confirm('Excluir registros selecionados?')) {
								...
								//parent.content.document.frm.action = "swing_excluir.php";
								//parent.content.document.frm.submit();
								document.frm.action = "swing_excluir.php";
								document.frm.submit();
							}
						}
						
						function gerar() {
							document.frm.action = "swing_arquivo.php";
							document.frm.submit();
						}						
					</script>	
					<?
						...
						if ($iSort) {
							...
							*/
							if ($iSort == 2) setSession("sOrder"," order by de" . $sDirection); 
							if ($iSort == 3) setSession("sOrder"," order by email" . $sDirection); 
							if ($iSort == 4) setSession("sOrder"," order by flagAtivo" . $sDirection); 
						}
					...
					?>
					<form name="form2" method="post" action="swing_lista.php">
						<div align="right">
						<table height="5px">
							<tr>
								<td class="LabelFONT">
					...
					<div class='acoes2'>
						<table width="100%">
							<tr>
								<td>
									<a class='botao' href='javascript:gerar()' target=''>&nbsp;Gerar Arquivo &nbsp;</a>				
					...
					<?
							...
							$table->addColumnHeader("Autor",true,"42%", "L"); // Título, Ordenar?, Largura, Alinhamento
							$table->addColumnHeader("E-mail",true,"50%", "L"); // Título, Ordenar?, Largura, Alinhamento
							$table->addColumnHeader("Ativo",true,"8%", "L"); // Título, Ordenar?, Largura, Alinhamento
							$table->addRow(); // adiciona linha (TR)
						
							while ($rs->getrow()) {
								$id = $rs->field("idSwings"); // captura a chave primária do recordset
								...
								$table->addData(addLink($rs->field("de"),"swing_edicao.php?id=$id&pagina=$pg","Clique para consultar ou editar registro"));
								$table->addData(addLink($rs->field("email"),"swing_edicao.php?id=$id&pagina=$pg","Clique para consultar ou editar registro"));								
								
								switch ($rs->field("flagAtivo")) {
									case "S":
										$flagAtivo = "SIM";
										break;
									case "N":
								...
								$table->addData(addLink($flagAtivo,"swing_edicao.php?id=$id&pagina=$pg","Clique para consultar ou editar registro"));								

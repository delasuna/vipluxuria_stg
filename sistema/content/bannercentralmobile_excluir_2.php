...
if (strlen($lista_exclusao) == 0) { // se não existe registros selecionados
    ...
    $sql = "DELETE FROM bannercentralmobile2 WHERE idBannerCentralMobile2 IN (" . $lista_exclusao . ")";
    $conn->execute($sql);

    echo "<script>location.href='/sistema/content/bannercentralmobile_lista_2.php?clear=1';</script>";
}

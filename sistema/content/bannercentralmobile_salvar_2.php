...
/* diretório de upload */
$uploaddir = "upload_bannercentralmobile2/";
...
if (isset($_FILES['imagem']) && !empty($_FILES['imagem']['tmp_name'])) {
    ...
    $novoNome = uniqid('bannercentralmobile2_', true) . '.' . $extensao;
    $destinoArquivo = $uploaddir . $novoNome;

    // Move o arquivo
    if (move_uploaded_file($imagem['tmp_name'], $destinoArquivo)) {
        $caminho_imagem = $destinoArquivo; // caminho salvo no banco
    } else {
...
$sql->setTable("bannercentralmobile2");
$sql->setKey("idBannerCentralMobile2", getParam("id"), "Number");
...
if (strlen(getParam("id")) > 0) {
    ...
    $destino = "bannercentralmobile_lista_2.php?pagina=" . getParam("pagina");
} else {
    // INSERT
    ...
    $conn->execute($sql->getSQL());
    $destino = "bannercentralmobile_lista_2.php";
}

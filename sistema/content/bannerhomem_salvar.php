...
if (isset($_FILES['imagem']) && !empty($_FILES['imagem']['tmp_name'])) {
    ...
    $novoNome = uniqid('bannerhomem_', true) . '.' . $extensao;
...
$sql->setTable("bannerHomem");
$sql->setKey("idBannerHomem", getParam("id"), "Number");
...
if (strlen(getParam("id")) > 0) {
    ...
    $destino = "bannerhomem_lista.php?pagina=" . getParam("pagina");
} else {
    // INSERT

			    ...
			    <div id="coluna-esquerda-full">
					...
					<script language="JavaScript">
						function excluir() {
							if (confirm('Excluir registros selecionados?')) {
								...
								//parent.content.document.frm.action = "blog_excluir.php";
					...
					<div class='acoes2'>
						<table width="100%">
							<tr>
								<td>
									&nbsp; <a class='botao' href='mural_edicao.php' target=''>&nbsp;Novo&nbsp;</a>
					...
					<?
							...
							$table->addColumnHeader("Data Publicação",true,"30%", "L"); // Título, Ordenar?, Largura, Alinhamento
							$table->addColumnHeader("Nome",true,"40%", "L"); // Título, Ordenar?, Largura, Alinhamento
							$table->addColumnHeader("Aprovado",true,"30%", "L"); // Título, Ordenar?, Largura, Alinhamento
							$table->addRow(); // adiciona linha (TR)
						
							while ($rs->getrow()) {
								$id = $rs->field("idMural"); // captura a chave primária do recordset
								...
								$table->addData(addLink($rs->field("dataPublicacao"),"mural_edicao.php?id=$id&pagina=$pg","Clique para consultar ou editar registro"));
								$table->addData(addLink($rs->field("nome"),"mural_edicao.php?id=$id&pagina=$pg","Clique para consultar ou editar registro"));
								$table->addData(addLink($rs->field("flagAprovado"),"mural_edicao.php?id=$id&pagina=$pg","Clique para consultar ou editar registro"));

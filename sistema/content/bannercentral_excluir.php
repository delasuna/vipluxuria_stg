...
if (strlen($lista_exclusao) == 0) { // se não existe registros selecionados
    ...
    $sql = "DELETE FROM bannercentral2 WHERE idBannerCentral2 IN (" . $lista_exclusao . ")";
    $conn->execute($sql);

    echo "<script>location.href='/sistema/content/bannercentral_lista.php?clear=1';</script>";
}

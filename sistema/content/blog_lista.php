...
<div id="tudo">
    <div id="conteudo">
        ...
        <div id="titulo">
        	<div id="titulo-content"><h1>Blog</h1></div>
            <div class="traco"></div>
        </div>
			    ...
			    <div id="coluna-esquerda-full">
					...
					<script>
						function excluir() {
							if (confirm('Excluir registros selecionados?')) {
								document.frm.action = "blog_excluir.php";
								document.frm.submit();
							}
						}
					</script>	
					...
					<?php
						...
						if ($iSort) {
							...
							if ($iSort == 2) setSession("sOrder"," order by assunto " . $sDirection); 
							if ($iSort == 3) setSession("sOrder"," order by dataPublicacao " . $sDirection); 
						}

						$sql = "SELECT * FROM blog " . getSession("sOrder");
					...
					<form name="form2" method="post" action="blog_lista.php">
						<div align="right">
							<table>
								<tr>
					...
					<div class='acoes2'>
						<table width="100%">
							<tr>
								<td>
									<a class='botao' href='blog_edicao.php'>&nbsp;Novo&nbsp;</a>
					...
					<?php
						...
						$table->addColumnHeader("Assunto",true,"70%","L");
						$table->addColumnHeader("Data",true,"30%","L");
						$table->addRow();

						while ($rs->getrow()) {
							$id = $rs->field("idBlog");
							$table->addData("<input type=\"checkbox\" name=\"sel[]\" value=\"$id\">");
							$table->addData(addLink($rs->field("assunto"),"blog_edicao.php?id=$id&pagina=$pg","Editar registro"));
							$table->addData(addLink($rs->field("dataPublicacao"),"blog_edicao.php?id=$id&pagina=$pg","Editar registro"));
							$table->addRow();	
						}

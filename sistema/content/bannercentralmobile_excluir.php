...
if (strlen($lista_exclusao) == 0) { // se não existe registros selecionados
    ...
    $sql = "DELETE FROM bannercentralmobile WHERE idBannerCentralMobile IN (" . $lista_exclusao . ")";
    $conn->execute($sql);

    echo "<script>location.href='/sistema/content/bannercentralmobile_lista.php?clear=1';</script>";
}

...
if (anti_injection($_POST["amigoIndicado"]) == "S") {
	if (anti_injection($_POST["nomeQuemIndicou"]) != "" && 
		...
		$corpo .= "<BR> O seu amigo " . $nomeQuemIndicou . " est� lhe indicando o " . $nomeAnunciante;
	...
	if ($registros==0) {
		...
		$resultado = mysql_query($sql, $conexao);
						...
						if (anti_injection($_REQUEST["id"]) != "") {
							...
							$resultado = mysql_query($sql, $conexao);
							if(!$resultado){
								die("Imposs�vel visualizar os anunciantes: " . mysql_error() . '<br>');
							}
...
<meta name="description" content="Vip Lux�ria � um classificados de an�ncios de Garotos de programa Porto Alegre." />

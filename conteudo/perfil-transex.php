...
if (isset($_REQUEST['votacao']) && $_REQUEST['votacao'] === 'S') {
    ...
    if ($idReq > 0 && ($conexao instanceof mysqli)) {
        ...
        $stmt = $conexao->prepare("SELECT id FROM votacaotransex WHERE idTransex = ? AND ip = ?");
        ...
        if ($already == 0) {
            if (isset($_REQUEST['voto']) && $_REQUEST['voto'] === 'aprovado') {
                $upd = $conexao->prepare("UPDATE transex SET quantidadeVotos = IFNULL(quantidadeVotos, 0) + 1 WHERE idTransex = ?");
                $upd->bind_param('i', $idReq);
                $upd->execute();
                $upd->close();
            } elseif (isset($_REQUEST['voto']) && $_REQUEST['voto'] === 'reprovado') {
                // decrementa mas não permite negativo
                $upd = $conexao->prepare("UPDATE transex SET quantidadeVotos = IF(quantidadeVotos > 0, quantidadeVotos - 1, 0) WHERE idTransex = ?");
                $upd->bind_param('i', $idReq);
                $upd->execute();
                $upd->close();
            }
            // registra IP para não votar novamente
            $ins = $conexao->prepare("INSERT INTO votacaotransex (idTransex, ip) VALUES (?, ?)");
...
// ==================================================
// Buscar perfil da tabela 'transex' (id via REQUEST)
// ==================================================
$perfil = null;
if (!empty($_REQUEST['id']) && ($conexao instanceof mysqli)) {
    $idTransex = intval($_REQUEST['id']);
    $stmt = $conexao->prepare("SELECT * FROM transex WHERE flagAtivo = 'Sim' AND idTransex = ?");
    $stmt->bind_param('i', $idTransex);
    $stmt->execute();
